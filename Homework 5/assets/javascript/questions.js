...
//jquery function loops all the questions and choices and appends to the html 
...
for (let i = 0; i < game.trivia.length; i++) {
    $("#trivia").append("<div class='question'>" + game.trivia[i].question + "</div>" +
        ...
        // let currQuestion = game.trivia[i];
        // let qDiv = $('<div>').addClass('question').text(currQuestion.question)
        //     //<div class='question'> game.trivia[i].question </div>
        // console.log(qDiv)
        // $('#trivia').append(qDiv);


    // for (let j = 0; j < currQuestion.choices.length; j++) {
    //     let aChoice = $('<input>').addClass('button button' + (j + 1))
    //         .attr('type', 'radio')
    //         .attr('name', 'question' + j)
    //         .attr('value', j)
    //         .text(currQuestion.choices[j]);
    //     $('#trivia').append(aChoice, '<br>');
    // }
}
